...
export function Player({
  ...
  useEffect(() => {
    ...
    return () => stopRotation();
  }, [isPlaying, track?.id, startRotation, rotationValue]);

  
  const resolveTrackUrl = async (t: Track): Promise<string> => {
    if (t.provider === 'local') {
      // 1. If URI is already a file:// path, use it directly
      if (t.uri && (t.uri.startsWith('file://') || t.uri.startsWith('/'))) {
        return t.uri.startsWith('/') ? `file://${t.uri}` : t.uri;
      }
      // 2. Check AsyncStorage for the stored fileUri (should be file://)
      try {
        const raw = await AsyncStorage.getItem(`offline_${t.id}`);
        if (raw) {
          const { fileUri } = JSON.parse(raw);
          if (fileUri && (fileUri.startsWith('file://') || fileUri.startsWith('/'))) {
            return fileUri.startsWith('/') ? `file://${fileUri}` : fileUri;
          }
        }
      } catch {}
      // 3. Try MediaLibrary to get localUri (guaranteed file:// path)
      try {
        const assetId = t.id.toString().replace('local_', '');
        ...
        if (assetInfo?.localUri) {
          ...
          try {
            ...
            if (raw) {
              ...
              meta.fileUri = assetInfo.localUri;
              await AsyncStorage.setItem(`offline_${t.id}`, JSON.stringify(meta));
            }
          } catch {}
          return assetInfo.localUri;
        }
      } catch (err) {
        console.warn('[Player] MediaLibrary getAssetInfoAsync failed:', err);
      }
      // 4. Last resort: use whatever URI we have
      return t.uri || '';
    }
  ...
  useEffect(() => {
    const sub = TrackPlayer.addEventListener(Event.PlaybackActiveTrackChanged, async (event) => {
      ...
      if (queueIndex >= 0 && musicQueueRef.current.setCurrentIndex) {
        ...
        try {
          const { recordPlay } = require('@/lib/stats-tracker');
          const trackObj = musicQueueRef.current.tracks[queueIndex];
          if (trackObj) {
            recordPlay(trackObj, trackObj.duration || 0);
          }
        } catch (e) {
    ...
    queueBuildDebounceRef.current = setTimeout(async () => {
      ...
      try {
        if (isSameTrack) {
          ...
          const encodeFileUrl = (uri: string) => {
            if (uri.startsWith('file://')) {
              return 'file://' + uri.substring(7).split('/').map(encodeURIComponent).join('/');
            }
            return uri;
          };
          ...
          const currentItem = {
            ...
            url: encodeFileUrl(currentUrl),
            title: current.title || 'Unknown Title',
            artist: current.artist || 'Unknown Artist',
            artwork: MusicAPI.getOptimalImage(current.images) || undefined,
              ...
              await TrackPlayer.add([
                {
                  ...
                  artwork: MusicAPI.getOptimalImage(t.images) || undefined,
    ...
    <>
      <Swipeable
        onSwipeableLeftOpen={() => {
          TrackPlayer.stop();
          onPlayingChange(false);
          if (musicQueue?.setCurrentIndex) {
            musicQueue.setCurrentIndex(-1);
          }
        }}
        renderLeftActions={() => (
