...
import { fetchLyrics, LyricsData } from '../lib/lyrics-api';
...
export function FullScreenPlayer({
  ...
  useEffect(() => {
    ...
    const loadPlaylists = async () => {
      ...
      setPlaylists(pls);
      const preSel = pls
        .filter(pl => pl.trackIds.includes(String(track.id)))
        .map(pl => pl.name);
      setSelected(preSel);
    };
    loadPlaylists();
  ...
  const handleQueueToggle = useCallback(() => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    onClose();
    // setTimeout to allow the modal to close before opening the queue, preventing modal overlap issues on Android
    setTimeout(() => {
      onQueueToggle?.();
    }, 100);
  }, [onQueueToggle, onClose]);
  ...
  const handleAddToPlaylist = useCallback(async () => {
    ...
    try {
      ...
      const toRemove = playlists.filter(
        pl => !selected.includes(pl.name) && pl.trackIds.includes(String(track.id))
      );

      for (const pl of toRemove) {
        await PlaylistStorage.removeTrackFromPlaylist(String(track.id), pl.name);
      }
  ...
  const handleShareCard = async () => {
    ...
    try {
      ...
      if (!isAvailable) {
        showToast('Sharing is not available', 'error');
        return;
      }
      showToast('Generating Social Card...', 'success');
      
      const fileUri = FileSystem.documentDirectory + 'social_card.jpg';
      const imageUrl = MusicAPI.getOptimalImage(track.images);
      if (imageUrl) {
  ...
  const LyricsModalView = (
    ...
    >
      <BlurView intensity={80} tint={isDark ? 'dark' : 'light'} style={styles.modalOverlay}>
        <SafeAreaView style={[styles.lyricsModalBox, { backgroundColor: isDark ? '#121212cc' : '#ffffffcc' }]}>
          <View style={styles.lyricsModalHeader}>
            <TouchableOpacity onPress={() => setShowLyricsModal(false)} style={styles.lyricsModalClose}>
              <Ionicons name="chevron-down" size={32} color={theme.textPrimary} />
            </TouchableOpacity>
            <Text style={[styles.modalTitle, { color: theme.textPrimary, marginBottom: 0 }]}>Lyrics</Text>
            <View style={{ width: 32 }} />
          </View>
          {fetchingLyrics ? (
            <View style={styles.lyricsCenter}>
              <Text style={{ color: theme.textPrimary }}>Loading lyrics...</Text>
            </View>
          ) : (
            <LyricsView lyricsData={lyrics} textColor={theme.textSecondary} activeColor={theme.textPrimary} />
          )}
        </SafeAreaView>
      </BlurView>
    </Modal>
  ...
  const InfoModalView = (
    ...
    >
      <BlurView intensity={40} tint={isDark ? 'dark' : 'light'} style={styles.modalOverlay}>
        <View style={[styles.modalBox, { backgroundColor: isDark ? '#1c1c24' : '#fff' }]}>
          <Text style={[styles.modalTitle, { color: theme.textPrimary }]}>Track Info</Text>
          <View style={{ width: '100%', marginBottom: 16 }}>
            <Text style={{ color: theme.textSecondary, fontSize: 12 }}>Title</Text>
            <Text style={{ color: theme.textPrimary, fontSize: 16, marginBottom: 8 }}>{track?.title || 'Unknown'}</Text>
            
            <Text style={{ color: theme.textSecondary, fontSize: 12 }}>Artist</Text>
            <Text style={{ color: theme.textPrimary, fontSize: 16, marginBottom: 8 }}>{track?.artist || 'Unknown'}</Text>
            
            <Text style={{ color: theme.textSecondary, fontSize: 12 }}>Album</Text>
            <Text style={{ color: theme.textPrimary, fontSize: 16, marginBottom: 8 }}>{track?.albumTitle || 'Unknown'}</Text>
            
            <Text style={{ color: theme.textSecondary, fontSize: 12 }}>Provider</Text>
            <Text style={{ color: theme.textPrimary, fontSize: 16, marginBottom: 8 }}>{track?.provider || 'Unknown'}</Text>
            
            {track?.releaseDate && (
              <>
                <Text style={{ color: theme.textSecondary, fontSize: 12 }}>Release Date</Text>
                <Text style={{ color: theme.textPrimary, fontSize: 16, marginBottom: 8 }}>{track.releaseDate}</Text>
              </>
            )}
          </View>
          <TouchableOpacity
          ...
          >
            <Text style={[styles.modalCancelText, { color: theme.textPrimary }]}>Close</Text>
          </TouchableOpacity>
        </View>
      </BlurView>
    </Modal>
        ...
        <View style={[styles.modalBox, { backgroundColor: isDark ? '#1c1c24' : '#fff' }]}>
          ...
          {sleepTimerActive ? (
            <View style={{ alignItems: 'center', marginBottom: 20 }}>
              <Text style={{ color: theme.textPrimary, fontSize: 18, marginBottom: 10 }}>
                {timeLeftMs ? formatTime(timeLeftMs) : ''} remaining
              </Text>
              <TouchableOpacity
            ...
            <View style={{ width: '100%' }}>
              {[15, 30, 45, 60].map((mins) => (
                <TouchableOpacity
                  key={mins}
                  style={[styles.modalItem, { backgroundColor: theme.glass, justifyContent: 'center' }]}
                  onPress={() => {
          ...
          <Animated.View style={[styles.mainContent, { opacity: fadeAnim }]}>
            ...
            <View style={styles.header}>
              ...
              <Text style={[styles.headerTitle, { color: theme.textPrimary }]}>
                {isDriveMode ? 'Drive Mode' : (t('player.now_playing') || 'Now Playing')}
              </Text>

              <TouchableOpacity
                onPress={() => setIsDriveMode(!isDriveMode)}
                style={[styles.headerButton, { backgroundColor: isDriveMode ? theme.accent : theme.glass }]}
              >
                <Ionicons name="car" size={24} color={isDriveMode ? '#fff' : theme.icon} />
              </TouchableOpacity>
            </View>

            {isDriveMode ? (
              <View style={styles.driveModeContent}>
                <Text style={[styles.driveModeTitle, { color: theme.textPrimary }]} numberOfLines={1}>
                  {decodeHtmlEntities(track.title)}
                </Text>
                <Text style={[styles.driveModeArtist, { color: theme.textSecondary }]} numberOfLines={1}>
                  {decodeHtmlEntities(track.artist)}
                </Text>
                <View style={styles.driveModeControls}>
                  <TouchableOpacity onPress={handlePrevious} style={[styles.driveModeBtn, { backgroundColor: theme.glass }]}>
                    <Ionicons name="play-skip-back" size={48} color={theme.icon} />
                  </TouchableOpacity>
                  <TouchableOpacity onPress={handlePlayPause} style={[styles.driveModeBtnPlay, { backgroundColor: theme.accent }]}>
                    <Ionicons name={isPlaying ? 'pause' : 'play'} size={64} color="#fff" />
                  </TouchableOpacity>
                  <TouchableOpacity onPress={handleNext} style={[styles.driveModeBtn, { backgroundColor: theme.glass }]}>
                    <Ionicons name="play-skip-forward" size={48} color={theme.icon} />
                  </TouchableOpacity>
                </View>
              </View>
            ) : (
              ...
              <View style={[
                ...
                <View style={[
                  ...
                  <TouchableOpacity
                    ...
                    style={styles.miniButtonWithText}
                  >
                    <Ionicons name="share-social" size={24} color={theme.textSecondary} />
                    <Text style={[styles.miniButtonText, { color: theme.textSecondary }]}>
                      Share
                    </Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity
                    ...
                    style={styles.miniButtonWithText}
                  >
                    <Ionicons name="moon" size={24} color={sleepTimerActive ? theme.accent : theme.textSecondary} />
                    <Text style={[styles.miniButtonText, { color: sleepTimerActive ? theme.accent : theme.textSecondary }]}>
                      Timer
                    </Text>
                  </TouchableOpacity>
