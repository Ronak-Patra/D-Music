...
export function useMusicQueue() {
  ...
  const queueModifiedBeforeLoad = useRef(false);
    ...
    const loadQueue = async () => {
      try {
        ...
        if (stored) {
          ...
          if (parsed.tracks && typeof parsed.currentIndex === 'number' && !queueModifiedBeforeLoad.current) {
            setQueue(parsed);
          }
        }
      } catch (error) {
  ...
  const setQueueTracks = useCallback((tracks: Track[], startIndex: number = 0) => {
    if (!isInitialized.current) queueModifiedBeforeLoad.current = true;
    setQueue({
      tracks,
